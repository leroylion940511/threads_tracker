...
| 週 | 主題 | 主要產出 |
|----|------|---------|
| W1 | Apify 可行性驗證 | 拿到 token、跑通關鍵字 search 模式、確認免費額度消耗速度 |
| W2 | 探索層 + DB 重構 | 建立新 schema、關鍵字種子池、每 60 分鐘輪詢、純記錄候選 |
| W3 | 評分層 | 硬規則 + Haiku + 加權；用 W2 累積資料盲測 prompt |
| W4 | 候選排程層 + 推送層 | 每日 09:00 Top 5、inline button、即時破例推送 |
| W5 | 收藏追蹤層 | 升格邏輯、分級輪詢、四類後續事件偵測 |
| W6 | 問答層 | `/ask` 對話模式、重量 context 組裝、`/exit` 與 timeout |
| W7 | 評估資料蒐集 + 調優 | 收藏率、後續命中率統計；prompt / 閾值調整 |
| W8 | 報告與 demo | 撰寫報告、3–5 個案例分析、demo 影片 |

W1 仍是最關鍵的一週——沒拿到 Apify token 之前，後面全是空轉。W1 結束沒拿到資料就要重新評估專題範圍。
...
### 風險二：Apify 額度燒得比預期快
W1 驗證階段就要算清楚 search 模式的 quota 消耗。應對：探索頻率可降、種子池可精簡。
